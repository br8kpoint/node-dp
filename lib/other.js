...
{"source":"dpotherinfoudf","name":"OTHER_REGISTER_UA","type":"varchar","nullable":false,"max":null,"label":"Registration User Agent","description":"User Agent used at time of Registration"},
...
/**
* Saves other info to donor perfect
...
OtherInfo.prototype.save = function (callback){
    ...
    parser.on('opentag', function(n){
        if(n.name === 'field') {
            self.other_id = parseInt(n.attributes.value);
        }
    })
    parser.on('end', function(){
        callback && callback(null, self);
    })
    var uri = dpURI('dp_saveotherinfo', params)
...
OtherInfo.getCount = function(sql, callback){
    ...
    request(dpURI(sql)).on('error', function(e){
        if(!finished){ finished = true; callback && callback(e, null); }
    }).pipe(parser)
}
...
OtherInfo.handleSaxOpenTag = function(n, context){
    if(n.name === 'field'){
        ...
        if(!otherinfo) return dpUtils.noteEnvelopeField(context, n.attributes); // field outside any <record>: DP error envelope, reported at 'end'
...
function getBySql(ids, callback){
    ...
    parser.on('end', function(){ console.log("finished other info..."); var e = dpUtils.envelopeError(otherinfos); e ? callback(e, null) : callback(null, otherinfos)}) // return the one array of otherinfos
