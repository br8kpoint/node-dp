...
Gift.getByDonorId = function(ids, callback){
    var sql = "select * from dpgift join dpgiftudf on dpgift.gift_id = dpgiftudf.gift_id where dpgift.donor_id = " + ids + " order by doguft,gift_date desc";
...
Gift.handleSaxOpenTag = function(n, context){
    if(n.name === 'field'){
        ...
        if(n.attributes.name in gift) gift[n.attributes.name] = n.attributes.value;
        else gift.udfFields[n.attributes.name] = n.attributes.value;
    }
...
function getBySql(ids, callback){
    ...
    parser.on('end', function(){ callback(null, gifts)}) // return the one and only donor specified by the id
