...
{"source":"dpgift","name":"CALLCENTER_PROCESSED","type":"varchar","nullable":false,"max":null,"label":"Call Center Processed","description":"Gift has been processed by call center"},
{"source":"dpgift","name":"CALLER","type":"varchar","nullable":false,"max":null,"label":"Ministry Caller","description":"Ministry Caller"},
{"source":"dpgift","name":"CALLER_EMAIL_LINK","type":"varchar","nullable":false,"max":null,"label":"Processed via email link","description":"Processed via email link sent from call center"},
{"source":"dpgift","name":"CC_LAST_CONTACT_DATE","type":"datetime","nullable":true,"max":null,"label":"Last Contact Date From Call Center","description":"Date and time donor was last contacted from call center"},
{"source":"dpgift","name":"SUBSCRIP_PROFILE_ID","type":"varchar","nullable":true,"max":null,"label":"Subscription Profile Id","description":"Subscription Profile Id in payment system"},
...
Gift.prototype.save = function (callback){
    callback = dpUtils.once(callback) // a request timeout/error and a stream end can both report; the caller hears once
    try{
        this.validate();
    }
    catch(ex){
    ...
    request(uri).on('error', function(e){
        ...
        if(dpUtils.settings.debug)  console.log("Response: "+response.statusCode); // 200
        if(dpUtils.settings.debug)  console.log("Data: " + response.data); // data
      }).pipe(parser);
}


Gift.prototype.savePledge = function(callback){
    callback = dpUtils.once(callback) // a request timeout/error and a stream end can both report; the caller hears once
    ...
    var uri = dpURI('dp_savepledge', params)
    request(uri).on('error', function(e){
        callback && callback(e, self)
      }).on('response', function(response) {
        if(dpUtils.settings.debug)  console.log("Response: "+response.statusCode); // 200
...
/**
 * remove a gift or pledge from dp
 * @param {Number} id the id of the pledge or gift
 * @param  {Function} cb the callback to execute with the results of the call
 * @return {[type]}      [description]
 */
Gift.remove = function(id, cb){
    cb = dpUtils.once(cb) // a request timeout/error and a stream end can both report; the caller hears once
    var self = this;
    var parser = require('sax').createStream(false, {lowercasetags: true});
    parser.on('error', function(e){
        cb(e, self)
...
Gift.getByDonorId = function(ids, callback){
    async.auto({
        gifts:function(callback){
            var sql = "select * from dpgift join dpgiftudf on dpgift.gift_id = dpgiftudf.gift_id where dpgift.record_type in ('G','M') and dpgift.donor_id = " + ids + " order by dpgift.gift_date desc";
            getBySql(sql, function(err, gifts){
                if(err){
                    console.log('Error retriving gifts:')
                    console.log(err);
                    callback(err, null);
                    return;
                }
        ...
        pledges:function(callback){
            var sql = "select * from dpgift join dpgiftudf on dpgift.gift_id = dpgiftudf.gift_id where dpgift.record_type = 'P' and dpgift.donor_id = " + ids + " order by dpgift.gift_date desc";
            getBySql(sql, function(err, gifts){
                if(err){
                    console.log('Error retriving pledges:')
                    console.log(err);
                    callback(err, null);
                    return;
                }
        ...
        if(Array.isArray(results.gifts)){
            if(Array.isArray(results.pledges)){
                callback(null, results.gifts.concat(results.pledges));
            }
        }
    })
...
Gift.query = function(options, callback){
    try
    {
        ...
        if(dpUtils.settings.debug)console.log(sql)
        var countsql = "select count(*) from " + from.join(", ") + where;
        if(dpUtils.settings.debug)console.log(countsql);
...
Gift.prototype.saveUdf = function(gift_id, callback){
    ...
    var update = "UPDATE dpgiftudf set "
...
Gift.getCount = function(sql, callback){
    ...
    request(dpURI(sql)).on('response', function(response) {
        if(dpUtils.settings.debug)  console.log("Response: "+response.statusCode); // 200
        if(dpUtils.settings.debug){
            var data = '';
          ...
          if(dpUtils.settings.debug) console.log(JSON.stringify(err));
          if(!finished){ finished = true; callback && callback(err, null); }
      }).pipe(parser)
}
...
Gift.handleSaxOpenTag = function(n, context){
    if(n.name === 'field'){
        ...
        if(!gift) return dpUtils.noteEnvelopeField(context, n.attributes); // field outside any <record>: DP error envelope, reported at 'end'
...
function getBySql(ids, callback){
    ...
    var parser = require('sax').createStream(false, {lowercasetags: true});
    var gifts = [];
    callback = dpUtils.once(callback)
    parser.on('error', function(e){ callback(e, null)})
    parser.on('opentag', function(n){Gift.handleSaxOpenTag(n, gifts)})
    parser.on('end', function(){ var e = dpUtils.envelopeError(gifts); e ? callback(e, null) : callback(null, gifts)}) // return the one array of gifts
