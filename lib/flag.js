...
function getBySql(ids, callback){
    ...
    parser.on('end', function(){ var e = dpUtils.envelopeError(flags); e ? callback(e, null) : callback(null, flags)}) // return the one array of flags
...
Flag.handleSaxOpenTag = function(n, context){
    if(n.name === 'field'){
        ...
        if(!flag) return dpUtils.noteEnvelopeField(context, n.attributes); // field outside any <record>: DP error envelope, reported at 'end'
