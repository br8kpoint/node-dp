...
Contact.getByDonorId = function(ids, callback){
    var sql = "select * from dpcontact join dpcontactudf on dpcontact.contact_id = dpcontactudf.contact_id where dpcontact.donor_id = " + ids + " order by dpcontact.contact_date desc";
...
Contact.handleSaxOpenTag = function(n, context){
    if(n.name === 'field'){
        ...
        if(n.attributes.name in contact){
            var field = Contacts.FIELDS.filter(function(item){
