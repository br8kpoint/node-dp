...
Contact.prototype.save = function (callback){
    ...
    parser.on('opentag', function(n){
        if(n.name === 'field') self.contact_id = parseInt(n.attributes.value);
    })
...
Contact.getByDonorId = function(ids, callback){
    var sql = "select * from dpcontact join dpcontactudf on dpcontact.contact_id = dpcontactudf.contactid where dpcontact.donor_id = " + ids + " order by dpcontact.contact_date desc";
...
Contact.handleSaxOpenTag = function(n, context){
    if(n.name === 'field'){
        ...
        if(n.attributes.name in contact) donor[n.attributes.name] = n.attributes.value;
...
function getByID(ids, callback){
    var sql = "select * from dpcontact join dpcontactudf on dpcontact.contact_id = dpcontactudf.conatct_id where dpcontact.contact_id = " + ids;
    getBySql(sql, function(err, contacts){
        if(err){
            callback(err, null);
            return;
