...
Contact.prototype.save = function (callback){
    ...
    try{
        ...
        // a validation failure (e.g. no donor id) is the caller's error, not a process crash:
        // this used to throw synchronously into an Express callback = uncaughtException (2026-09-21)
        return callback(new Error(ex.message || ex), null)
    }
    ...
    request(dpURI('dp_savecontact', params)).on('error', function(e){
...
Contact.handleSaxOpenTag = function(n, context){
    if(n.name === 'field'){
        ...
        if(!contact) return dpUtils.noteEnvelopeField(context, n.attributes); // field outside any <record>: DP error envelope, reported at 'end'
...
function getBySql(ids, callback){
    ...
    parser.on('end', function(){ var e = dpUtils.envelopeError(contacts); e ? callback(e, null) : callback(null, contacts)}) // return the results
