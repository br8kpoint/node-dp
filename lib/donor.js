...
Donor.prototype.save = function (callback){
    ...
    request(dpURI('dp_savedonor', params)).on('response', function(response) {
...
Donor.query = function(options, callback){
    ...
    if(dpUtils.settings.debug)console.log(options);
...
function getBySql(ids, callback){
    ...
    if(dpUtils.settings.debug)console.log("returning donors: ", donors);
    parser.on('end', function(){
        if(dpUtils.settings.debug) console.log(JSON.stringify(donors)); 
