...
const request = require('request').defaults({ timeout: 20000 }) /* DP normally answers in < 1 s; a stalled connection must become an error, not a hung caller */;
...
Donor.UDFFIELDS = [
	...
	{"source":"dpudf","name":"WALLET_TYPE","type":"varchar","nullable":true,"max":200,"label":"Wallet Type","description":"Apple/Google Pay wallet marker (APPLE/GOOGLE)"},
    ];
Donor.JOIN_MAP = {
    "dpudf": {
        "source": "dp.donor_id",
...
Donor.prototype.save = function(callback) {
    const promise = new Promise((resolve, reject) => {
        ...
        let url = 
        request(dpURI('dp_savedonor', params)).on('error', err => reject(err)).on('response', response => {
...
function getBySql(sql) {
    return new Promise((resolve, reject) => {
        ...
        parser.on('end', () => { const e = dpUtils.envelopeError(donors); e ? reject(e) : resolve(donors); });

        request(dpURI(sql)).on('error', e => reject(e)).pipe(parser);
    });
}
...
Donor.handleSaxOpenTag = function(n, context) {
    if (n.name === 'field') {
        ...
        if (donor) {
            ...
            dpUtils.noteEnvelopeField(context, n.attributes); // field outside any <record>: DP error envelope, reported at 'end'
        }
