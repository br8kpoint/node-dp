...
Donor.UDFFIELDS = [
    ...
    {"source":"dpudf","name":"EVANGELIST_NAME","type":"varchar","nullable":true,"max":200,"label":"Evangelist Name","description":"Evangelist Name"},
...
Donor.prototype.saveUdf = function(callback){
    ...
    for(var i = 0; i < Donor.UDFFIELDS.length; i++){
        update += Donor.UDFFIELDS[i].name + ' = ' + this.udfFields[Donor.UDFFIELDS[i].name] + ","
    }
    //remove comma at end
    update = update.substring(0, update.length - 1)
    update += "where donor_id = " + this.donor_id
...
Donor.prototype.save = function (callback){
    ...
    parser.on('end', function(){
        Donor.saveUdf(function(err, result){
...
Donor.prototype.getGifts = function(callback){
    gifts.getGyDonorId(this.donor_id, function(err, gifts){
